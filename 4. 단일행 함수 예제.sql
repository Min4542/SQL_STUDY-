...
SELECT 
    ...
    SUBSTR('SQL Developer', 5,2 ), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번) 공백문자도 포함
    LENGTH('HELLO WORLD'),         -- 문자열의 길이 (공백문자도 포함)
    TRIM('    HI   ')              -- 좌우 공백 제거
FROM dual;
...
--    ,REPLACE('Java Programmer', 'Java') AS "REPLACE" -- 문자를 변경 (제거 기능도 있음)
FROM dual;
...
SELECT 
    ...
    SIGN(50) AS SIGN, --0보다 작으면 -1, 0보다 크면 1, 0이면 0 음수, 양수 구별용
    ABS(-20) AS ABS --절대값
FROM dual;
